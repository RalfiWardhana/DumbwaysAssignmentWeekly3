       ...
       connection.query(query,(err,results) => {
           ...
           for(let i = 0 ; i < results.length ; i++){
               if((results[i].email == email) && (results[i].password == password) ) {
                   if((results[i].email == "admin@gmail.com") && (results[i].password == "admin123")){
...
app.post("/register", (req,res) =>{
    ...
    if((nameRegister == "") || (emailRegister == "") || (passwordRegister == "")){
        ...
        const query = `INSERT INTO tb_users(email,name,password) VALUES ("${emailRegister}", "${nameRegister}", "${passwordRegister}")`
            connection.query(query,(err,results)=>{
            if (err) throw err;
            req.session.message = {
                type : "success",
...
app.post("/",(req,res) => {
    ...
    if(movieGenre == ""){
        ...
        const queryy = `SELECT * FROM tb_types JOIN tb_movies ON tb_types.id = tb_movies.type_id WHERE tb_types.name = "${movieGenre}"`
...
app.get("/pesan-payment/:id",(req,res) => {
    const id = req.params.id;
    const query = `SELECT * FROM tb_movies JOIN tb_tickets ON tb_movies.id = tb_tickets.movie_id WHERE tb_movies.id = ${id}`
    connection.query(query, (err,results) => {
      if (err) throw err;
...
app.post("/payment",(req,res)=>{
    ...
    const query = `INSERT INTO tb_payments(amount,sub_total,ticket_id) VALUES (${clearMovie[0].amount}, ${clearMovie[0].subtotal},${clearMovie[0].id})`
    connection.query(query,(err,results)=>{
    if (err) throw err;
    req.session.message = {
         type : "success",
...
app.post("/add",upload.single('image') ,(req,res) =>{
    ...
    const movieId= convertStringToNumber(`${req.body.addMovieId}`);
    const typeId= convertStringToNumber(`${req.body.addTypeId}`);
    const date = req.body.addDate;
    const time = req.body.addTime;

    const query = `INSERT INTO tb_movies(name,movie_hour,photo,type_id) VALUES ("${title}", "${hour}","${photo}",${typeId})`
        connection.query(query,(err,results)=>{
        if (err) throw err;
        req.session.message = {
             type : "success",
    ...
    const queryy = `INSERT INTO tb_tickets(date_show,time_show,price,venue,movie_id) VALUES ("${date}", "${time}", "${price}","${venue}",${movieId})`
        connection.query(queryy,(err,results)=>{
        if (err) throw err;
        req.session.message = {
             type : "success",
...
app.post("/edit",upload.single('image'),(req,res) =>{
    ...
    const query = `UPDATE tb_movies SET name = "${title}", movie_hour = "${hour}", photo = "${photo}", type_id = ${typeId} WHERE id = ${id}`
        connection.query(query,(err,results)=>{
        if (err) throw err;
        req.session.message = {
             type : "success",
    ...
    const queryy = `UPDATE tb_tickets SET date_show = "${date}", time_show = "${time}", price = "${price}", venue = "${venue}", movie_id=${movieId} WHERE movie_id = ${id}`
        connection.query(queryy,(err,results)=>{
        if (err) throw err;
        req.session.message = {
             type : "success",
...
app.get("/delete/:id", (req,res) => {
    ...
    connection.query(query, (err,results) => {
     ...
     const queryy= `DELETE FROM tb_tickets WHERE movie_id=${id}`;
     connection.query(queryy, (err,results) => {
      if (err) throw err;
      console.log(results);
     })
...
app.get("/list", (req,res) => {
    const query = `SELECT * FROM tb_tickets JOIN tb_payments ON tb_tickets.id = tb_payments.ticket_id`
    connection.query(query, (err,results) => {
      if (err) throw err;
      console.log(results);
